...
function waitForChatResult(params: {
  ...
  return new Promise((resolve, reject) => {
    const timer = window.setTimeout(() => {
      unsubscribe();
      reject(new Error("yyVideoClaw tool call timed out"));
    }, params.timeoutMs);
    const unsubscribe = params.client.addEventListener((evt: GatewayEventFrame) => {
      ...
      if (payload.state === "final") {
        ...
        unsubscribe();
        resolve(extractTextFromMessage(payload.message) || "yyVideoClaw finished with no text.");
      } else if (payload.state === "error") {
        window.clearTimeout(timer);
        unsubscribe();
        reject(new Error(payload.errorMessage ?? "yyVideoClaw tool call failed"));
      }
    });
  });
}
