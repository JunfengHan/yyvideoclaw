...
export function applySettingsFromUrl(host: SettingsHost) {
  ...
  if (hasTokenParam) {
    if (queryToken != null) {
      warnQueryToken = true;
      console.warn(
        "[yyvideoclaw] Auth token passed as query parameter (?token=). Use URL fragment instead: #token=<token>. Query parameters may appear in server logs.",
      );
    }
...
function buildAttentionItems(host: SettingsAppHost) {
  ...
  if (auth?.scopes && !hasOperatorReadAccess(auth)) {
    items.push({
      ...
      href: "https://docs.yyvideoclaw.ai/web/dashboard",
      external: true,
    });
  }
