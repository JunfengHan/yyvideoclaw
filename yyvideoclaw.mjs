...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `yyvideoclaw: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["yyvideoclaw: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:yyvideoclaw/yyvideoclaw#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g yyvideoclaw@latest`.");
  return lines.join("\n");
};
